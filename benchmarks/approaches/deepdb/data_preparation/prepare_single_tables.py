...
def prepare_table_wrapper(args):
    # Unpack arguments
...
def prepare_all_tables(schema_graph, path, csv_seperator=',', max_table_data=20000000):
    ...
    args = [(schema_graph, table_obj, path, csv_seperator, max_table_data) 
            for table_obj in schema_graph.tables]
    
    # Use number of CPUs minus 1 to avoid overwhelming the system
    n_processes = 24# max(1, cpu_count() - 1)
    
    # Create pool and map tasks
    with Pool(processes=n_processes) as pool:
        results = pool.map(prepare_table_wrapper, args)
