...
class DBConnection:
    ...
    def get_result(self, sql):
        ...
        # connect to "dbname=imdb user=USER password=PW host=127.0.0.1 port=5431"
        # hardcoded connection string for IMDB database
        connection = psycopg2.connect("dbname=imdb user=USER password=PW host=127.0.0.1 port=5431")
        #connection = psycopg2.connect(user=self.db_user,
        #                              password=self.db_password,
        #                              host=self.db_host,
        #                              port=self.db_port,
