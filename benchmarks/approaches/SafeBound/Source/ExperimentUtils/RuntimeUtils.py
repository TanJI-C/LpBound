...
def generate_subqueries_sql(queryJGs, outputFile, statsFile):
    #  return a csv file with QueryID, Tables, SQL
    stats = pickle.load(open(statsFile, "rb"))
    subqueries_sqls = ["QueryID|Tables|SQL|Estimate|InferenceTime"]
    for query_id, queryJG in enumerate(queryJGs):
        for subQuery in queryJG.getIntermediateQueries():

            inferenceStart = datetime.now()
            # estimate the size of the subQuery
            estimate = stats.functionalFrequencyBound(subQuery)
            inferenceEnd = datetime.now()
            inference_time = (inferenceEnd - inferenceStart).total_seconds()

            tables = " ".join(list(subQuery.vertexDict.keys()))
            sql = subQuery.getSQLQuery(countStar=False)
            subqueries_sqls.append(f"{query_id}|{tables}|{sql}|{estimate}|{inference_time}")

    result = "\n".join(subqueries_sqls)
    with open(outputFile, "w") as f:
        f.write(result)


def evaluate_runtime_safe_bound(statsFile, benchmark, queryJGs, dbConn, outputFile, performTableScan, runs):
...
def evaluate_runtime_postgres(benchmark, queryJGs, dbConn, statisticsTarget, outputFile, runs):
...
def evaluate_runtime_bayes_card(ensembleDirectory, benchmark, queryJGs, dbConn, outputFile, runs):
    dbConn.changeStatisticsTarget(100)
    schema = None
    ...
    for j in range(runs):
        ...
        for i, queryJG in enumerate(queryJGs):
            ...
            parsedSubQueries, _ = prepare_join_queries(
                schema, ensembleDirectory, pairwise_rdc_path=None, query_filename=tempSQLFilePath, true_card_exist=False
            )
...
def evaluate_runtime_true_cardinality(statsFile, benchmark, queryJGs, dbConn, outputFile, runs):
    dbConn.changeStatisticsTarget(100)
    queryLabels = []
    runLabels = []
    inferenceTimes = []
...
def evaluate_runtime_pessimistic_cardinality_estimation(statsFile, benchmark, queryJGs, dbConn, outputFile, runs):
    dbConn.changeStatisticsTarget(100)
    queryLabels = []
    runLabels = []
    inferenceTimes = []
...
def evaluate_runtime_neurocard(benchmark, queryJGs, dbConn, outputFile, runs):
    ...
    hintDF = pd.read_csv(
        "/home/ec2-user/FrequencyBounds/Workloads/NeuroCardHintIndex_" + benchmark + ".csv",
        delimiter="|",
        names=["QueryLabel", "Tables"],
    )
    estimatesDF = pd.read_csv(
        "/home/ec2-user/FrequencyBounds/Data/Results/NeuroCard_Inference_" + benchmark + "SubQueries.csv"
    )
...
def evaluate_runtime(
    ...
    if benchmark == "JOBLight":
        ...
        queryJGs = SQLFileToJoinQueryGraphs(rootFileDirectory + "Workloads/jobjoinQueries.sql")
        # dbConn = getDBConn("jobjoin")

    if method == "generate_subqueries":
        generate_subqueries_sql(queryJGs, outputFile, statsFile)
        return

    if method == "SafeBound":
        evaluate_runtime_safe_bound(
            statsFile, benchmark, queryJGs, dbConn, outputFile, performTableScan=False, runs=runs
        )
    elif method == "SafeBoundScan":
        evaluate_runtime_safe_bound(
            statsFile, benchmark, queryJGs, dbConn, outputFile, performTableScan=True, runs=runs
        )
        ...
        evaluate_runtime_pessimistic_cardinality_estimation(
            statsFile, benchmark, queryJGs, dbConn, outputFile, runs=runs
        )
