...
def clean_groupby_sql(sql_file_path: str) -> str:
    file = open(sql_file_path)
    fileContents = file.read()
    file.close()

    cleaned_file_contents = ""
    # clean the file contents for groupby queries
    # for each line, clean the query
    for line in fileContents.split("\n"):
        cleaned_file_contents += clean_sql_query(line) + ";" + "\n"

    return cleaned_file_contents


def subqueries_csv_to_sql(csv_file_path: str) -> str:
    # read the csv file
    df = pd.read_csv(csv_file_path, sep="|")
    # for each row, create a sql query
    sql_queries = []
    for index, row in df.iterrows():
        sql_queries.append(row["SQL"])
    return "\n".join(sql_queries)


def evaluate_inference_safe_bound(statsFile, benchmark, outputFile):
    ...
    if benchmark == "JOBLight":
        ...
        queryJGs = SQLFileToJoinQueryGraphs(rootDirectory + "Workloads/jobjoinQueries.sql")
    elif "groupby" in benchmark:
        bechmark_name_dict = {
            "joblight": "joblight",
            "joblightranges": "jobrange",
            "stats": "stats",
        }
        benchmark_name = bechmark_name_dict[benchmark.split("_")[0].lower()]
        variant = benchmark.split("_")[2]
        query_file_name = f"Workloads/{benchmark_name}_gby/{benchmark_name}_groupby_{variant}.sql"
        sql_queries = clean_groupby_sql(query_file_name)

        queryJGs = SQLQueriesToJoinQueryGraphs(sql_queries)
    elif "subqueries" in benchmark:
        benchmark_name_dict = {
            "joblight": "joblight",
            "joblightranges": "jobrange",
            "stats": "stats",
        }
        benchmark_name = benchmark_name_dict[benchmark.split("_")[0].lower()]
        query_file_name = f"Workloads/{benchmark_name}/{benchmark_name}_subqueries.csv"
        sql_queries = subqueries_csv_to_sql(query_file_name)
        # print(sql_queries)
        queryJGs = SQLQueriesToJoinQueryGraphs(sql_queries)
    ...
    for i, queryJG in enumerate(queryJGs):
        ...
        # print(i)
        # print(queryJG.getSQLQuery(countStar=False))
...
def evaluate_inference_simplicity(statsFile, benchmark, outputFile):
    stats = pickle.load(open(statsFile, "rb"))
    queryJGs = None
    dbConn = None
    if benchmark == "JOBLight":
...
def getRelevantPreds(joinQueryGraph, vertex, FKtoKDict):
    ...
    if vertex.tableName in FKtoKDict:
        ...
        for joinCol in vertex.outputJoinCols:
            ...
            equalityClassMembers = joinQueryGraph.equalityClasses[
                joinQueryGraph.equalityDict[vertex.alias + "." + vertex.tableName + "." + joinCol]
            ]
...
def evaluate_inference_postgres(benchmark, outputFile, statisticsTarget):

    IMDB_FKtoKDict = {
        "aka_title": [["movie_id", "id", "title"], ["kind_id", "id", "kind_type"]],
        "cast_info": [
            ["movie_id", "id", "title"],
        ...
        "movie_info": [["movie_id", "id", "title"], ["info_type_id", "id", "info_type"]],
        "movie_info_idx": [["movie_id", "id", "title"], ["info_type_id", "id", "info_type"]],
        "movie_keyword": [["movie_id", "id", "title"], ["keyword_id", "id", "keyword"]],
    ...
    if benchmark == "JOBLight":
        ...
        queryJGs = [
            removePKJoins(x, IMDB_FKtoKDict)
            for x in SQLFileToJoinQueryGraphs(rootDirectory + "Workloads/JOBLightQueries.sql")
        ]
        dbConn = getDBConn("imdb_fk_pk")
    elif benchmark == "JOBLightRanges_FK_PK":
        queryJGs = [
            removePKJoins(x, IMDB_FKtoKDict)
            for x in SQLFileToJoinQueryGraphs(rootDirectory + "Workloads/JOBLightRangesQueries.sql")
        ]
        dbConn = getDBConn("imdb_fk_pk")
    elif benchmark == "JOBM_FK_PK":
        queryJGs = [
            removePKJoins(x, IMDB_FKtoKDict)
            for x in SQLFileToJoinQueryGraphs(rootDirectory + "Workloads/JOBMQueries.sql")
        ]
        dbConn = getDBConn("imdb_fk_pk")
    elif benchmark == "Stats_FK_PK":
        queryJGs = [
            removePKJoins(x, Stats_FKtoKDict)
            for x in SQLFileToJoinQueryGraphs(rootDirectory + "Workloads/StatsQueries.sql")
        ]
...
def evaluate_inference_bayes_card(ensembleDirectory, benchmark, outputFile):
    sqlFile = None
    schema = None
    ...
    parsedQueries, true = prepare_join_queries(
        schema, ensembleDirectory, pairwise_rdc_path=None, query_filename=sqlFile, true_card_exist=True
    )
    modelSize = 0
...
def evaluate_inference_pessimistic_cardinality_estimation(benchmark, outputFile):
    ...
    for i, queryJG in enumerate(queryJGs):
        ...
        queryHints = []
        hintFile = (
            "/home/ec2-user/FrequencyBounds/pqo-opensource/output/results/"
            + benchmark
            + "_"
            + str(i)
            + "_SubQueryBounds.txt"
