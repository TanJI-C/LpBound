...
def build_safe_bound(benchmark, parameters, outputFile):
    ...
    if benchmark == "jobjoin":
        ...
        joinColumns = [
            ["person_id"],  # aka_name
            ["kind_id", "movie_id"],  # aka_title
            ["movie_id", "person_id", "person_role_id", "role_id"],  # cast_info
            ["id"],  # char_name
            ["id"],  # comp_cast_type
            ["id"],  # company_name
            ["id"],  # company_type
            ["movie_id", "status_id", "subject_id"],  # complete_cast
            ["id"],  # info_type
            ["id"],  # keyword
            ["id"],  # kind_type
            ["id"],  # link_type
            ["movie_id", "company_id", "company_type_id"],  # movie_companies
            ["movie_id", "info_type_id"],  # movie_info
            ["movie_id", "info_type_id"],  # movie_info_idx
            ["movie_id", "keyword_id"],  # movie_keyword
            ["movie_id", "linked_movie_id", "link_type_id"],  # movie_link
            ["id"],  # name
            ["info_type_id", "person_id"],  # person_info
            ["id"],  # role_type
            ["id", "kind_id"],  # title
        ]
        ...
        for i, table in enumerate(tableNames):
            cols = list(set(joinColumns[i] + filterColumns[i]))
            data_table = data[table][cols]
            tables.append(data_table)

        del data

        FKtoKDict = {}  # no FK to K dict for this benchmark

    elif benchmark == "JOBLight":
        data = load_imdb()

        tableNames = ["cast_info", "movie_companies", "movie_info_idx", "movie_info", "movie_keyword", "title"]

        joinColumns = [["movie_id"], ["movie_id"], ["movie_id"], ["movie_id"], ["movie_id"], ["id", "kind_id"]]
        ...
        FKtoKDict = {
            "nation": [["n_regionkey", "r_regionkey", "region"]],
            "supplier": [["s_nationkey", "n_nationkey", "nation"]],
            "customer": [["c_nationkey", "n_nationkey", "nation"]],
            "partsupp": [["ps_partkey", "p_partkey", "part"], ["ps_suppkey", "s_suppkey", "supplier"]],
...
def build_stats_object(
    method="SafeBound", benchmark="JOBLight", parameters=dict(), outputFile="../../StatObjects/SafeBound_JOBLight.pkl"
):
