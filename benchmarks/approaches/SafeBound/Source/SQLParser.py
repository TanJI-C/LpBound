...
def getSQLGrammar():
    ...
    columnRval = (realNum | intNum | quoteStr | aliasAndColumnName).setName(
        "column_rvalue"
                ...
                if isJoin:
                    ...
                    if predType == "IN":
                        if clause[3][1].getName() == "intNum":
                            queryGraph.addPredicate(
                                clause[0], clause[1], predType, [int(x[0]) for x in clause[3][1:-1]]
                            )
                        elif clause[3][1].getName() == "realNum":
                            queryGraph.addPredicate(
                                clause[0], clause[1], predType, [float(x[0]) for x in clause[3][1:-1]]
                            )
                        else:
                            queryGraph.addPredicate(
                                clause[0], clause[1], predType, [str(x[0]) for x in clause[3][1:-1]]
                            )
...
def clean_sql_query(sql_query: str) -> str:
    """
    Cleans a single SQL query string by:
    1. Replacing 'SELECT DISTINCT(...)' with 'SELECT *'.
    2. Removing all 'AND col IS NOT NULL' clauses.
    """
    ...
    # 1. Replace SELECT DISTINCT(...) with SELECT *
    # This is case-insensitive
    modified_query = re.sub(r"SELECT DISTINCT\([^)]*\)", "SELECT *", query, flags=re.IGNORECASE)

    # 2. Remove all instances of 'AND col IS NOT NULL'
    # This is case-insensitive
    modified_query = re.sub(r"\s+AND\s+[\w\.]+\s+IS\s+NOT\s+NULL", "", modified_query, flags=re.IGNORECASE)
