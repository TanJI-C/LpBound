...
from DeepDBUtils.aqp_spn.group_by_combination import group_by_combinations
from DeepDBUtils.ensemble_compilation.spn_ensemble import CombineSPN
from DeepDBUtils.rspn.algorithms.ranges import NominalRange, NumericRange
from DeepDBUtils.rspn.rspn import RSPN
from DeepDBUtils.rspn.structure.leaves import IdentityNumericLeaf, identity_distinct_ranges, categorical_distinct_ranges, \
    Categorical, identity_likelihood_range, categorical_likelihood_range
from DeepDBUtils.rspn.updates.top_down_updates import cluster_center_update_dataset
...
class AQPSPN(CombineSPN, RSPN):
    ...
    def _parse_conditions(self, conditions, group_by_columns=None, group_by_tuples=None):
        ...
        for (table, condition) in conditions:
            ...
            print(f"matching_fd_cols: {matching_fd_cols}")
