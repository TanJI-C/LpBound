...
def prepare_single_table(schema_graph, table, path, max_distinct_vals=10000, csv_seperator=",",
    ...
    for relationship_obj in incoming_relationships:
        ...
        assert table_primary_key == left_attribute, "Currently, only references to primary key are supported. Current primary key: {table_primary_key}, current left attribute: {left_attribute}".format(table_primary_key=table_primary_key, left_attribute=left_attribute)
...
# Move process_table outside of prepare_all_tables
def process_table(args):
    schema_graph, table_obj, path, csv_seperator, max_table_data = args
    table = table_obj.table_name
    return table, prepare_single_table(schema_graph, table, path + '/' + table + '.hdf',
...
def prepare_all_tables(schema_graph, path, csv_seperator=',', max_table_data=20000000):
    ...
    num_processes = 23 #max(1, cpu_count() - 1)
    
    # Create argument tuples for each table
    args_list = [(schema_graph, table_obj, path, csv_seperator, max_table_data) 
                 for table_obj in schema_graph.tables]
    
    # Create a process pool and map the work
    with Pool(processes=num_processes) as pool:
        # Process tables in parallel and collect results
        results = pool.map(process_table, args_list)
        
        # Convert results to dictionary
        meta_data = dict(results)
