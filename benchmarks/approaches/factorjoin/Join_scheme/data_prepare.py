...
from Schemas.stats.schema import gen_stats_light_schema, gen_dblp_schema
from Schemas.imdb.schema import gen_imdb_schema, gen_job_light_imdb_schema,gen_job_range_imdb_schema, gen_imdb_job_join_schema
...
def process_stats_data(data_path, model_folder, n_bins=500, bucket_method="greedy", save_bucket_bins=False,
                       return_bin_means=False, get_bin_means=False, actual_data=None, dataset='stats'):
    """
    ...
    for table_obj in schema.tables:
        ...
        for attr in df_rows.columns:
            if attr in all_keys:
                ...
                values = df_rows[attr].values
                
                # Convert to numeric if possible, handling non-numeric values
                try:
                    values = pd.to_numeric(values, errors='coerce')
                except:
                    # If conversion fails, treat non-numeric values as -1
                    values = np.where(pd.isna(values), -1, values)
                
                key_data[attr] = values
                # Handle NaN and negative values
                key_data[attr] = np.where(pd.isna(key_data[attr]), -1, key_data[attr])
                key_data[attr] = np.where(key_data[attr] < 0, -1, key_data[attr])
                
                null_values[table_name][attr] = -1
                key_data[attr] = copy.deepcopy(key_data[attr])[key_data[attr] >= 0]
                
                # if all keys have exactly one appearance, consider them primary keys
                # set error margin of 0.01 for data mis-write
                ...
                temp = np.where(pd.isna(temp), null_values[table_name][attr], temp)
...
def process_imdb_data(data_path, model_folder, n_bins, bucket_method, sample_size=1000000, save_bucket_bins=False,
                      seed=0):
    schema = gen_imdb_job_join_schema(data_path)
    all_keys, equivalent_keys = identify_key_values(schema)
...
def process_table(args):
    """Helper function to process individual tables in parallel"""
    table_obj, all_keys = args
    ...
    for attr in df_rows.columns:
        if attr in all_keys:
            # Convert to numeric first to handle mixed types safely
            data = pd.to_numeric(df_rows[attr].values, errors='coerce')
            data = np.where(np.isnan(data), -1, data)
            data = np.where(data < 0, -1, data)

            table_results['na_values'][attr] = len(data[data != -1]) / table_results['table_lens']

            filtered_data = copy.deepcopy(data)[data >= 0]
            table_results['data'][attr] = filtered_data

            if len(np.unique(filtered_data)) >= len(filtered_data) - 10:
                table_results['primary_keys'].append(attr)

            attr_value_count = np.unique(filtered_data, return_counts=True)[-1]
            table_results['table_key_count_var'][attr] = np.var(attr_value_count)
            
    return table_obj.table_name, table_results

def process_imdb_data_parallel(data_path, model_folder, n_bins, bucket_method, sample_size=1000000, save_bucket_bins=False,
                             seed=0, n_processes=24):
    """Parallelized version of process_imdb_data"""
    import multiprocessing as mp
    # Use spawn context to avoid SIGSEGV with pandas/numpy in forked processes
    ...
    process_args = [(table_obj, all_keys) for table_obj in schema.tables]

    # Process tables in parallel using spawn context
    with ctx.Pool(processes=n_processes) as pool:
        results = pool.map(process_table, process_args)
    ...
    for table_name, result in results:
        table_lens[table_name] = result['table_lens']
        data.update(result['data'])
        
        if table_name not in na_values:
            na_values[table_name] = {}
        na_values[table_name].update(result['na_values'])
        
        table_key_count_var.update(result['table_key_count_var'])
        primary_keys.extend(result['primary_keys'])
