...
class Bound_ensemble:
    ...
    def parse_query_simple(self, query):
        ...
        for table_str in tables_str.split(","):
            ...
            if " as " in table_str_lower:
                # Handle both "AS" and "as" - split case-insensitively
                parts = table_str_lower.split(" as ")
                alias = parts[-1].strip()
                table_name = table_str[:table_str_lower.index(" as ")].strip()
                tables_all[alias] = table_name
            else:
                parts = table_str.split(" ")
                tables_all[parts[-1].lower()] = parts[0]
        ...
        for cond in conditions:
            ...
            if not join:
                ...
                if "date" in attr.lower():
                    # Handle both PostgreSQL format ('2014-09-11'::timestamp) and plain format ('2014-09-11')
                    if "::timestamp" in str(value):
                        value = timestamp_transorform(value.strip().split("::timestamp")[0].strip("'\""))
                    else:
                        # Plain format - strip quotes and convert
                        value = timestamp_transorform(str(value).strip("'\""))
    ...
    def get_all_id_conidtional_distribution_bn(self, table_queries, join_keys, equivalent_group):
        ...
        for table in join_keys:
            ...
            if table in table_queries:
                ...
                table_query = {}
            print("FUNCTION: get_all_id_conidtional_distribution_bn")
            print(table_query)
            print(key_attrs)
    ...
    def eliminate_one_key_group(self, tables, key_group, factors, relevant_keys):
        ...
        # --> this does not work for imdb when using the standard settings!
        rest_group = None
