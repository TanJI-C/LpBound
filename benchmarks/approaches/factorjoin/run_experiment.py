...
if __name__ == '__main__':
    ...
    parser.add_argument('--db_conn_kwargs', type=str,
                        #default=f"dbname=imdb user={DB_USER} password={DB_PASSWORD} host={DB_HOST} port={DB_PORT}",
    ...
    if args.dataset == 'stats':
        if args.preprocess_data:
            ...
            #write training time to a file
            with open('training_time_stats.txt', 'w') as f:
                f.write(f"Training completed: total training time is {end_time - start_time}")
        ...
        if args.generate_models:
            ...
            print(f"Training completed: total training time is {end_time - start_time}")
            #write training time to a file
            with open('training_time_jobrange.txt', 'w') as f:
                f.write(f"Training completed: total training time is {end_time - start_time}")
            ...
            test_on_imdb(args.model_path, args.query_file_location, args.derived_query_file, args.sampling_percentage,
                         args.query_sample_location, save_file, args.db_conn_kwargs)
        ...
        if args.preprocess_data:
            ...
            save_file = os.path.join(args.save_folder, "stats_dblp_queries_" +
                                     args.model_path.split("/")[-1].split(".pkl")[0] + ".txt")
            test_on_stats(args.model_path, args.query_file_location, save_file)
