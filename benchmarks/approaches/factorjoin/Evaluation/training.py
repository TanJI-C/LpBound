...
import os
import numpy as np
from Join_scheme.data_prepare import process_stats_data, process_imdb_data, process_imdb_data_parallel
...
def test_trained_BN_on_dblp(bn, t_name):
    queries = {
        "edge": "SELECT COUNT(*) FROM edge as e",
        "vertex": "SELECT COUNT(*) FROM vertex as v"
    }
...
def train_one_stats(dataset, data_path, model_folder, n_dim_dist=2, n_bins=200, bucket_method="greedy",
    ...
    data, null_values, key_attrs, table_buckets, equivalent_keys, schema, bin_size = process_stats_data(data_path,
                                        model_folder, n_bins, bucket_method, save_bucket_bins, actual_data=actual_data, dataset=dataset)
    all_bns = dict()
    for table in schema.tables:
        ...
        if validate:
            if dataset == 'dblp':
                test_trained_BN_on_dblp(bn, t_name)
            elif dataset == 'stats':
...
def train_one_imdb(data_path, model_folder, n_dim_dist=1, bin_size=None, bucket_method="fixed_start_key",
    ...
    if not os.path.exists(model_folder):
        os.mkdir(model_folder)
    schema, table_buckets, ground_truth_factors_no_filter, bins, equivalent_keys = process_imdb_data_parallel(data_path,
