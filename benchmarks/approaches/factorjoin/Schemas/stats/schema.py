...
def gen_dblp_schema(hdf_path):
    schema = SchemaGraph()

    schema.add_table(Table("edge",
                           primary_key=[],
                           attributes=['s', 't'],
                           irrelevant_attributes = [],
                           no_compression=[],
                           csv_file_location=hdf_path.format('edge'),
                           table_size=2099732
                           ))
    
    schema.add_table(Table("vertex",
                           primary_key=['i'],
                           attributes=['i', 'l', 'd'],
                           irrelevant_attributes=['d'],
                           csv_file_location=hdf_path.format('vertex'),
                           table_size=317080
                            ))
    
    schema.add_relationship('edge', 's', 'vertex', 'i')
    schema.add_relationship('edge', 't', 'vertex', 'i')
